...
To access STARR-OMOP-confidential and STARR-OMOP-confidential-lite databases, please follow the steps [here](getting_access.qmd).
...
The same tables and columns are also published as a [Data Dictionary](omop_data_dictionary.qmd) — one searchable, sortable, filterable grid per table, with the whole model in a single **All Fields** view. It is generated from the same dbt models, so it never falls behind the data model page.

The complete dictionary is also downloadable as an Excel workbook, one sheet per table.

[View Data Dictionary →](omop_data_dictionary.qmd){.btn .btn-primary}
[Download as Excel (.xlsx)](downloads/starr_omop_data_dictionary.xlsx){.btn .btn-outline-primary}
